...
export const register = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (user) return next(new ApiError("Email already registered", 400));
    if (findUsername) return next(new ApiError("Username already exist", 400));
...
export const loginWithCredential = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!match) return next(new ApiError("Wrong password", 400));
...
export const checkMe = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user: IUser = await User.findById(req.user._id).select("-password -refreshToken -passwordResetExpires -passwordResetToken -__v");
    res.status(200).json({
      success: true,
      data: {
        user,
